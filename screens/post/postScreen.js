        ...
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: Colors.blackColor }}>
                ...
                <View style={{ flex: 1 }}>
                    {this.header()}
                    {this.postInfo()}
                    {this.selectCoverInfo()}
                    {this.commentOnInfo()}
                    {this.saveToGalleryInfo()}
                </View>
                {this.postVideoButton()}
            </SafeAreaView>
        )
    }
    ...
    saveToGalleryInfo() {
        return (
            <View style={styles.saveToGalleryInfoWrapStyle}>
                ...
                <Switch
                    value={this.state.saveToGallery}
                    onValueChange={(val) => { this.setState({ saveToGallery: val }) }}
                    disabled={false}
                    activeText={'On'}
    ...
    commentOnInfo() {
        return (
            <View style={styles.commentOnInfoWrapStyle}>
                ...
                <Switch
                    value={this.state.commentOn}
                    onValueChange={(val) => { this.setState({ commentOn: val }) }}
                    disabled={false}
                    activeText={'On'}
                    activeTextStyle={{ ...Fonts.whiteColor16Bold }}
                    inactiveTextStyle={{ ...Fonts.whiteColor16Bold }}
        ...
        return (
            <View style={styles.postInfoWrapStyle}>
                ...
                <TextInput
                    ...
                    multiline
                    numberOfLines={6}
                    value={this.state.aboutPost}
                    onChangeText={(text) => this.setState({ aboutPost: text })}
