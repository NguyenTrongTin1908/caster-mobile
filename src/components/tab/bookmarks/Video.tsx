...
const Video = ({ route }: IProps) => {
  ...
  const loadBookmarkPosts = async (more = false, refresh = false) => {
    ...
    let videoData = data.data.filter(item => item?.objectInfo?.type !== "photo")
    ...
    return (
      ...
      <View key={item._id}>
        <Image
          ...
          source={ item.objectInfo?.files[0].thumbnails[0] ?{ uri: item.objectInfo?.files[0].thumbnails[0] } : require("../../../assets/bg.jpg")}
        />
      </View>):
      (null)
    }
  ...
  return (
    <ScrollView >
      <View style={{ marginHorizontal: Sizes.fixPadding - 15.0, flexDirection: 'column', flexWrap: 'wrap', marginVertical: 5 }}>
      <FlatList
          data={feeds}
