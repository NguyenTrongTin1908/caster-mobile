...
const conversationSagas = [
  ...
  },
  {
    ...
    *worker(data: IReduxAction<Record<string, string>>) {
      try {
        const { source, sourceId, conversationId } = data.payload;
        const conversationMapping = yield select(
          (state) => state.conversation.mapping
        );
        if (conversationId) {
          if (conversationMapping[conversationId]) {
            ...
            const readAllMessages = yield messageService.readAllInConversation(
              conversationId
            );
            if (readAllMessages) {
          ...
          if (conversationMapping[conversation._id]) {
            yield put(
              setActiveConversationSuccess(
                conversationMapping[conversation._id]
              )
            );
          } else {
  ...
  },
  {
    ...
    *worker(data: IReduxAction<Record<string, string>>) {
      try {
        ...
        const resp = yield messageService.deleteConversation(conversationId);
        const conversation = resp.data;
        if (conversationMapping[conversation._id]) {
          yield put(
            setActiveConversationSuccess(conversationMapping[conversation._id])
          );
        } else {
          yield put(setActiveConversationSuccess(conversation));
        }
  ...
  },
  {
    ...
    *worker(data: IReduxAction<Record<string, string>>) {
      try {
        const conversation = yield messageService.getConversationDetail(
          data.payload.id
        );
