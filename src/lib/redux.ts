...
/* tslint:disable */
function handleActions(actions: any, initialState: any): any {
  return handleReduxActions(
    reduce(
      actions,
      (reducer: any, handler, action) => {
        reducer[action] = (state: any, act: any) =>
...
function createSelectorsA(context: string, keys: any[] = []): any {
  ...
  return keys.map((key: any) => (state: any) =>
    isArray(key)
...
function createSelectors(context: string, keys: string[]): any {
  ...
  return reduce(
    keys,
    (selectors: any, key) => {
      selectors[`${key}Selector`] = (state: any) =>
        stateSelector(state).get(key);
