...
const Following = (props: IProps): React.ReactElement => {
  ...
  const loadPerformers = async (more = false, q = '', refresh = false) => {
    ...
    const { data } = await followService.search({
      ...
      limit: 10,
      sourceId: props.route.params.performerId ? props.route.params.performerId : '6245a9911ea6bb1b817e4874'
    });
  ...
  return (
    <Box flex={1} mx="auto" w="100%">
      <FlatList
        ...
        renderItem={({ item }) => ((item.targetInfo?._id && item.targetInfo._id !== props.current._id) ? (<PerformerCard performer={item.targetInfo} />) : null)}
